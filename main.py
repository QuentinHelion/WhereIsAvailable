...
ref_country = "fr"
ref_path = f"/shows/search/title?title={title_encoded}&country={ref_country}&output_language=fr"
...
for country_code in countries.keys():
    # Utilisation de l'endpoint /shows/search/filters avec catalogs=prime.subscription
    path = f"/shows/search/filters?country={country_code}&catalogs=prime.subscription&keyword={title_encoded}&output_language=fr"
    ...
    if isinstance(result, list):
        ...
        found = bool(result.get("shows"))
    if found:
        print(f"Trouvé sur Prime Video (abonnement) dans le pays : {country_code} ({countries[country_code]})")
        # print(json.dumps(result, indent=2, ensure_ascii=False))
        break
    else:
        print(f"Aucun résultat Prime Video (abonnement) pour {country_code} ({countries[country_code]})")
else:
